...
insertArgs:: [String]->[Val]->Env->Env
insertArgs (k:ks)(v:vs)env = insertArgs ks vs (H.insert k v env)
insertArgs _ _ env = env
...
--- ### If Expressions
eval (IfExp op a b) env = 
  case eval op env of 
    BoolVal True -> eval a env
    BoolVal False -> eval b env
    otherwise -> ExnVal "Condition is not a Bool"
  
--- ### Functions and Function Application
eval (FunExp x e1) env =
  CloVal x e1 env
eval (FunExp [] _)env = ExnVal "Apply to non-closure"

eval (AppExp e args) env =
 let  argVals = evalArgs args env 
     in case eval e env of
      CloVal cloArgs body cloEnv -> eval body (insertArgs cloArgs argVals cloEnv)
...
evalStmt (PrintStmt x) = eval x env 
evalStmt (SetStmt x) = "" 

evalSeqStmt :: [Stmt]->String
evalSeqStmt [] = ""
evalSeqStmt (x:xs)  = evalStmt x :  evalSeqStmt xs

exec (PrintStmt e) penv env = (val, penv, env)
    where val = show $ eval e env

--- ### Set Statement
exec (SetStmt var e) penv env = ("", penv, envl)
   where val = eval e env 
         envl = H.insert var val env

--- ### Sequencings
exec (SeqStmt []) penv env = ("", penv, env)
exec (SeqStmt (x:xs)) penv env = 
  let (s1, penv1, env1) = exec x penv env
      (s2, penv2, env2) = exec (SeqStmt xs) penvl env1
  in (s1++s2, penv2, env2)
...
exec (IfStmt e s1 s2) penv env = 
  case eval e env of 
      BoolVal True -> (s1, penv, env) = exec s1 penv env
      BoolVal False -> (s2, penv, env) = exec s2 penv env
      otherwise -> (ExnVal "Condition is not a Bool") penv env

--- ### Procedure and Call Statements
exec p@(ProcedureStmt name args body) penv env = ("", penvl, env)
    where penvl = H.insert name p penv 

--exec (CallStmt name args) penv env = 
--    let Just (ProcedureStmt _ params body) = H.lookup name penv
--        in exec body args penv env
